...
/** P0 gate for the next provider. One Piece episode 1 is the baseline case. */
@RunWith(AndroidJUnit4::class)
class AnimasuProviderE2ETest {
    @Test
    fun onePieceEpisodeOneRendersFirstFrame() = runBlocking {
        val provider = RemoteSourceProvider("animasu", "Animasu", 30, "animasu")
        val results = provider.search("One Piece")
        assertFalse("Animasu search returned no results", results.isEmpty())
        val anime = results.firstOrNull {
            it.title.trim().equals("One Piece", true) || it.id.contains("one-piece", true)
        }
        assertNotNull("Animasu search did not return canonical One Piece", anime)

        val selectedAnime = anime!!
        assertNotNull("Animasu detail resolution failed for ${selectedAnime.id}", provider.getAnime(selectedAnime.id))
        val episodes = provider.getEpisodes(selectedAnime.id)
        assertFalse("Animasu returned no episodes for ${selectedAnime.id}", episodes.isEmpty())
        val episode = episodes.firstOrNull { it.number == 1 }
        assertNotNull("Animasu episode 1 is not present for ${selectedAnime.id}", episode)

        val streams = provider.getStreams(selectedAnime.id, episode!!.number)
        assertFalse("Animasu returned no streams for episode 1", streams.isEmpty())
        ...
        val selected = streams
            ...
            .firstOrNull()
        assertNotNull("Animasu returned no HTTP(S) stream URL", selected)
        assertTrue("Animasu selected an UNKNOWN stream type", selected!!.type != StreamType.UNKNOWN)
    ...
    private fun renderFirstFrame(stream: ProviderStream) {
        ...
        val thread = HandlerThread("KakaAnime-Animasu-E2E").apply { start() }
    ...
    private companion object { const val TAG = "KakaAnime-Animasu-E2E" }
}
