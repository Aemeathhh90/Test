...
/** P0 gate for Nimegami. One Piece episode 1 is the baseline case. */
@RunWith(AndroidJUnit4::class)
class NimegamiProviderE2ETest {
    @Test
    fun onePieceEpisodeOneRendersFirstFrame() = runBlocking {
        val provider = RemoteSourceProvider("nimegami", "Nimegami", 250, "nimegami")
        val results = provider.search("One Piece")
        assertFalse("Nimegami search returned no results", results.isEmpty())
        val anime = results.firstOrNull {
            it.title.trim().equals("One Piece", true) || it.id.contains("one-piece", true)
        }
        assertNotNull("Nimegami search did not return canonical One Piece", anime)

        val selectedAnime = anime!!
        assertNotNull("Nimegami detail resolution failed for ${selectedAnime.id}", provider.getAnime(selectedAnime.id))
        val episodes = provider.getEpisodes(selectedAnime.id)
        assertFalse("Nimegami returned no episodes for ${selectedAnime.id}", episodes.isEmpty())
        val episode = episodes.firstOrNull { it.number == 1 }
        assertNotNull("Nimegami episode 1 is not present for ${selectedAnime.id}", episode)

        val streams = provider.getStreams(selectedAnime.id, episode!!.number)
        assertFalse("Nimegami returned no streams for episode 1", streams.isEmpty())
        ...
        val selected = streams
            ...
            .firstOrNull()
        assertNotNull("Nimegami returned no HTTP(S) stream URL", selected)
        assertTrue("Nimegami selected an UNKNOWN stream type", selected!!.type != StreamType.UNKNOWN)
    ...
    private fun renderFirstFrame(stream: ProviderStream) {
        ...
        val thread = HandlerThread("KakaAnime-Nimegami-E2E").apply { start() }
    ...
    private companion object { const val TAG = "KakaAnime-Nimegami-E2E" }
}
