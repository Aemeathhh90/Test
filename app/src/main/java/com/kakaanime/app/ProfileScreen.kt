...
fun ProfileScreen(themeState: KakaThemeState, monetizationState: MonetizationState, onPremiumClick: () -> Unit) {
    ...
    if (showAnimeWatched) {
        AnimeWatchedScreen(preferences, localAnime, { showAnimeWatched = false }) { showAnimeWatched = false }
        return
    }
    if (showEpisodeWatched) {
        EpisodeWatchedScreen(preferences, localAnime, { showEpisodeWatched = false }) { _, _ -> showEpisodeWatched = false }
        return
    }
    if (editingProfile) {
    ...
    LazyColumn(Modifier.fillMaxSize().background(MaterialTheme.colorScheme.background), contentPadding = PaddingValues(18.dp, 14.dp, 18.dp, 116.dp), verticalArrangement = Arrangement.spacedBy(14.dp)) {
        item { Column { Text("Account", 28.sp, FontWeight.Bold); Text("Manage your account and preferences", 12.sp, color = MaterialTheme.colorScheme.onSurfaceVariant) } }
        item {
            Surface(Modifier.fillMaxWidth(), RoundedCornerShape(24.dp), color = MaterialTheme.colorScheme.surfaceVariant.copy(alpha = .58f)) {
                Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                    Row(verticalAlignment = Alignment.CenterVertically) {
                        Box(Modifier.size(68.dp).clip(CircleShape).background(profileAccent), contentAlignment = Alignment.Center) { Text(initials, fontWeight = FontWeight.Black, fontSize = 19.sp, color = Color.White) }
                        Spacer(Modifier.size(12.dp))
                        Column(Modifier.weight(1f)) {
                            Text(profileName, 18.sp, FontWeight.Bold)
                            Text(profileBio, 11.sp, color = MaterialTheme.colorScheme.onSurfaceVariant, maxLines = 2)
                            Spacer(Modifier.height(6.dp))
                            Surface(shape = RoundedCornerShape(9.dp), color = if (monetizationState.isPremium) Color(0xFFFFB52E).copy(alpha = .16f) else MaterialTheme.colorScheme.surface) { Text(if (monetizationState.isPremium) "Premium" else "Free User", Modifier.padding(horizontal = 9.dp, vertical = 5.dp), 10.sp, FontWeight.Bold, color = if (monetizationState.isPremium) Color(0xFFFFB52E) else MaterialTheme.colorScheme.onSurfaceVariant) }
                        }
                    ...
                    Surface(Modifier.fillMaxWidth(), RoundedCornerShape(14.dp), color = MaterialTheme.colorScheme.background.copy(alpha = .55f)) {
                        Row(Modifier.padding(horizontal = 13.dp, vertical = 10.dp), verticalAlignment = Alignment.CenterVertically) { Icon(Icons.Outlined.Star, null, tint = MaterialTheme.colorScheme.primary, modifier = Modifier.size(18.dp)); Spacer(Modifier.size(8.dp)); Text("${monetizationState.diamonds} Diamonds", Modifier.weight(1f), fontWeight = FontWeight.SemiBold, fontSize = 13.sp); Text("Premium", 11.sp, color = MaterialTheme.colorScheme.primary, modifier = Modifier.clickable(onClick = onPremiumClick)) }
                    }
                }
            }
        }
        item {
            Surface(Modifier.fillMaxWidth().clip(RoundedCornerShape(20.dp)).clickable(onClick = onPremiumClick), RoundedCornerShape(20.dp), color = MaterialTheme.colorScheme.primary.copy(alpha = .12f)) {
                Row(Modifier.padding(17.dp), verticalAlignment = Alignment.CenterVertically) { Box(Modifier.size(46.dp).clip(CircleShape).background(MaterialTheme.colorScheme.primary.copy(alpha = .16f)), contentAlignment = Alignment.Center) { Icon(Icons.Outlined.Star, null, tint = MaterialTheme.colorScheme.primary) }; Spacer(Modifier.size(12.dp)); Column(Modifier.weight(1f)) { Text(if (monetizationState.isPremium) "Premium Aktif" else "Upgrade to Premium", FontWeight.Bold, 16.sp); Text("1080p, Auto Skip, dan fitur premium lainnya", 11.sp, color = MaterialTheme.colorScheme.onSurfaceVariant) }; Text("›", 28.sp, color = MaterialTheme.colorScheme.primary) }
            }
        }
        item {
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(9.dp)) {
                AccountStat("Anime", preferences.loadWatchedEpisodes().size, Modifier.weight(1f)) { showAnimeWatched = true }
    ...
    if (showAppearance) AlertDialog(onDismissRequest = { showAppearance = false }, title = { Text("Appearance") }, text = { Column(verticalArrangement = Arrangement.spacedBy(14.dp)) { Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) { Icon(Icons.Outlined.DarkMode, null); Spacer(Modifier.size(10.dp)); Text("Dark mode", Modifier.weight(1f)); Text(if (themeState.darkMode) "ON" else "OFF", color = MaterialTheme.colorScheme.primary, modifier = Modifier.clickable { themeState.darkMode = !themeState.darkMode; preferences.saveDarkMode(themeState.darkMode) }) }; Text("Accent color", style = MaterialTheme.typography.titleSmall); Row(horizontalArrangement = Arrangement.spacedBy(7.dp)) { KakaAccent.entries.forEach { accent -> Surface(Modifier.weight(1f).height(34.dp).clickable { themeState.accent = accent; preferences.saveAccentName(accent.name) }, RoundedCornerShape(9.dp), color = accent.primary.copy(alpha = if (themeState.accent == accent) .92f else .24f)) {} } } } }, confirmButton = { TextButton(onClick = { showAppearance = false }) { Text("Selesai") } })
...
@Composable
private fun AccountSection(content: @Composable ColumnScope.() -> Unit) { Surface(Modifier.fillMaxWidth(), RoundedCornerShape(22.dp), color = MaterialTheme.colorScheme.surfaceVariant.copy(alpha = .45f)) { Column(content = content, modifier = Modifier.padding(vertical = 4.dp)) } }

@Composable
private fun AccountRow(icon: androidx.compose.ui.graphics.vector.ImageVector, title: String, subtitle: String, onClick: () -> Unit) { Row(Modifier.fillMaxWidth().clickable(onClick = onClick).padding(horizontal = 15.dp, vertical = 13.dp), verticalAlignment = Alignment.CenterVertically) { Icon(icon, null, tint = MaterialTheme.colorScheme.onSurfaceVariant, modifier = Modifier.size(22.dp)); Spacer(Modifier.size(13.dp)); Column(Modifier.weight(1f)) { Text(title, fontWeight = FontWeight.SemiBold, fontSize = 14.sp); Text(subtitle, 10.sp, color = MaterialTheme.colorScheme.onSurfaceVariant) }; Text("›", 23.sp, color = MaterialTheme.colorScheme.onSurfaceVariant) } }

@Composable
private fun AccountStat(label: String, value: Int, modifier: Modifier = Modifier, onClick: (() -> Unit)? = null) { Surface(modifier.then(if (onClick != null) Modifier.clickable(onClick = onClick) else Modifier), RoundedCornerShape(16.dp), color = MaterialTheme.colorScheme.surfaceVariant.copy(alpha = .48f)) { Column(Modifier.padding(vertical = 13.dp), horizontalAlignment = Alignment.CenterHorizontally) { Text(value.toString(), 18.sp, FontWeight.Bold); Text(label, 10.sp, color = MaterialTheme.colorScheme.onSurfaceVariant) } } }
