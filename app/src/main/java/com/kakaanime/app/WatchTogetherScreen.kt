...
fun WatchTogetherScreen(onBack: () -> Unit) {
    ...
    Surface(
        ...
        LazyColumn(
            ...
            item {
                Row(
                    ...
                    Surface(shape = CircleShape, color = MaterialTheme.colorScheme.surfaceVariant) {
                        Text("?", Modifier.padding(horizontal = 11.dp, vertical = 7.dp), fontWeight = FontWeight.Bold)
                    }
                }
            }
            item {
            ...
            if (selectedTab == 0) {
                item {
                    Text("Active Rooms · ${activeRooms.size} room tersedia", style = MaterialTheme.typography.titleLarge, fontWeight = FontWeight.Bold)
                }
                items(activeRooms, key = { it.id }) { room ->
                    WatchRoomCard(room) { selectedRoom = room }
                }
        ...
        AlertDialog(
            ...
            confirmButton = {
                TextButton(
                    ...
                    onClick = {
                        val room = activeRooms.firstOrNull { it.id.equals(joinCode.trim(), ignoreCase = true) }
