...
class OtakudesuProvider : AnimeProvider {
    ...
    override suspend fun getAnime(animeId: String): ProviderAnime? {
        ...
        val primary=requestJson("$baseUrl/anime/${encodePath(slug)}")?.optJSONObject("data"); if(primary!=null) return primary.toProviderAnime(slug)
    ...
    override suspend fun getEpisodes(animeId: String): List<ProviderEpisode> {
        val slug=normalizeAnimeSlug(animeId); val web=webSource.getAnime(slug)
        if(web!=null){ val episodes=webSource.getEpisodes(web); if(episodes.isNotEmpty()) return episodes.map{e->ProviderEpisode(id="$id:${e.url}",animeId="$id:$slug",number=e.number,providerId=id,title=e.title)} }
    ...
    override suspend fun getStreams(animeId:String,episodeNumber:Int):List<ProviderStream>{
        ...
        if(episodeRef.startsWith("http",true)){val resolved=streamResolver.resolve(listOf(episodeRef),null);if(resolved.isNotEmpty())return resolved.map{it.copy(providerId=id)}}
        val episodeSlug=normalizeEpisodeSlug(episodeRef); val primary=requestJson("$baseUrl/episode/${encodePath(episodeSlug)}")
        if(primary!=null){val resolved=streamResolver.resolve(primary.optJSONObject("data")?.streamCandidates().orEmpty(),episodeRef);if(resolved.isNotEmpty())return resolved.map{it.copy(providerId=id)}}
        ...
        if(streamUrl.isNotBlank()){val resolved=streamResolver.resolve(listOf(streamUrl),episodeRef);if(resolved.isNotEmpty())return resolved.map{it.copy(providerId=id)}}
        return emptyList()
    }
    ...
    private fun JSONObject.toProviderAnime(slug:String)=ProviderAnime(id="$id:$slug",title=optString("title").ifBlank{"Unknown Anime"},providerId=id,posterUrl=optString("thumbnail").ifBlank{null},description=optJSONArray("synopsis")?.toStringList()?.joinToString("\n")?:optString("synopsis"),status=optJSONObject("metadata")?.optString("status").ifNullOrBlank("UNKNOWN"))
    ...
    private fun JSONArray.toProviderEpisodeList(slug:String):List<ProviderEpisode>=buildList{for(i in 0 until length()){val item=optJSONObject(i)?:continue;val endpoint=item.optString("slug").trim('/');val number=item.optString("episode").toIntOrNull()?:extractEpisodeNumber(item.optString("title"),endpoint)?:continue;add(ProviderEpisode(id="$id:${endpoint.ifBlank{"$slug-episode-$number-sub-indo"}}",animeId="$id:$slug",number=number,providerId=id,title=item.optString("title").ifBlank{"Episode $number"},thumbnailUrl=item.optString("thumbnail").ifBlank{item.optString("thumb").ifBlank{item.optString("image").ifBlank{null)}}))}}.sortedBy{it.number}
    private fun JSONArray.toLegacyProviderEpisodeList(slug:String):List<ProviderEpisode>=buildList{for(i in 0 until length()){val item=optJSONObject(i)?:continue;val endpoint=item.optString("endpoint").trim('/');val number=item.optString("episode").toIntOrNull()?:extractEpisodeNumber(item.optString("title"),endpoint)?:continue;add(ProviderEpisode(id="$id:${endpoint.ifBlank{"$slug-episode-$number-sub-indo"}}",animeId="$id:$slug",number=number,providerId=id,title=item.optString("title").ifBlank{"Episode $number"},thumbnailUrl=item.optString("thumbnail").ifBlank{item.optString("thumb").ifBlank{item.optString("image").ifBlank{null)}}))}}.sortedBy{it.number}
    private fun encode(value:String)=URLEncoder.encode(value,"UTF-8");private fun encodePath(value:String)=encode(value).replace("+","%20")
}
