...
class OtakudesuProvider : AnimeProvider {
    ...
    override suspend fun search(query: String): List<ProviderAnime> {
        ...
        val primary = requestJson("$baseUrl/search?q=${encode(normalized)}&page=1")
            ?.optJSONObject("data")
        ...
        return requestJson("$legacyUrl/search/${encode(normalized)}")
    ...
    override suspend fun getAnime(animeId: String): ProviderAnime? {
        ...
        val primary = requestJson("$baseUrl/anime/${encodePath(slug)}")
            ?.optJSONObject("data")
        if (primary != null) return primary.toProviderAnime(slug)

        return requestJson("$legacyUrl/anime/${encodePath(slug)}")
            ?.toLegacyProviderAnime(slug)
    }
    ...
    override suspend fun getEpisodes(animeId: String): List<ProviderEpisode> {
        ...
        return requestJson("$legacyUrl/anime/${encodePath(slug)}")
            ?.let { it.optJSONObject("anime_detail") ?: it }
    ...
    override suspend fun getStreams(
        ...
        val episode = getEpisodes(animeId)
        ...
        val streamUrl = legacy
            ?.let { it.optJSONObject("episode_detail") ?: it }
            ?.optString("stream_link")
            ?.trim()
            .orEmpty()
    ...
    private fun JSONObject.toProviderAnime(slug: String): ProviderAnime =
        ProviderAnime(
            id = "$id:$slug",
            title = optString("title").ifBlank { "Unknown Anime" },
            providerId = id,
            posterUrl = optString("thumbnail").ifBlank { null },
            description = optJSONArray("synopsis")?.toStringList()?.joinToString("\n")
                ?: optString("synopsis"),
            status = optJSONObject("metadata")?.optString("status").ifNullOrBlank("UNKNOWN")
        )
    ...
    private fun JSONObject.toLegacyProviderAnime(slug: String): ProviderAnime {
        ...
        return ProviderAnime(
            id = "$id:$slug",
            title = detail.optString("title").ifBlank { "Unknown Anime" },
            providerId = id,
            posterUrl = detail.optString("thumb").ifBlank { null },
            description = detail.optString("synopsis"),
            status = detail.optString("status").ifBlank { "UNKNOWN" }
        )
    }
    ...
    private fun JSONObject.streamCandidates(): List<String> = buildList {
        ...
        for (i in 0 until streams.length()) {
            ...
            item.optString("embedUrl").trim().takeIf { it.isNotBlank() }?.let(::add)
        }
        optString("defaultStreamUrl").trim().takeIf { it.isNotBlank() }?.let(::add)
    }.distinct()

    private fun normalizeAnimeSlug(value: String): String {
        val raw = value.removePrefix("$id:").trim().trim('/')
        val slug = raw.substringAfterLast("/anime/", raw)
            .substringBefore("?")
            .trim('/')
