...
class AllAnimeProvider : AnimeProvider {
    ...
    override suspend fun search(query: String): List<ProviderAnime> {
        ...
        return array(root).mapNotNull { item ->
            ...
            ProviderAnime(
                ...
                providerId = id,
                latestEpisode = maxOf(
                    item.optInt("episodes_sub", 0),
                    item.optInt("episodes_dub", 0)
                ).takeIf { it > 0 }
            )
        }
    }
    ...
    override suspend fun getAnime(animeId: String): ProviderAnime? {
        ...
        val title = firstString(root, "title", "name") ?: return null
        return ProviderAnime(
            id = "$id:$showId",
            title = title,
            providerId = id,
            description = firstString(root, "description", "synopsis") ?: "",
            posterUrl = firstString(root, "poster", "posterUrl", "image", "thumbnail"),
            backdropUrl = firstString(root, "backdrop", "cover"),
            genres = stringArray(root, "genres", "genre"),
            alternativeTitles = stringArray(root, "alternativeTitles", "alternatives"),
            year = firstString(root, "year", "release", "released")?.let {
                Regex("\\b(19\\d{2}|20\\d{2})\\b").find(it)?.value?.toIntOrNull()
            },
            status = firstString(root, "status") ?: "UNKNOWN"
        )
    }
    ...
    override suspend fun getEpisodes(animeId: String): List<ProviderEpisode> {
        ...
        return array(root).mapNotNull { item ->
            val number = when {
                item.has("episode") -> item.optInt("episode", 0)
                item.has("number") -> item.optInt("number", 0)
                else -> item.optString("episodeNumber").toIntOrNull() ?: 0
            }
            ...
            ProviderEpisode(
                ...
                number = number,
                providerId = id,
                title = item.optString("title").trim().ifBlank { "Episode $number" }
            )
    ...
    override suspend fun getStreams(animeId: String, episodeNumber: Int): List<ProviderStream> {
        val showId = animeId.substringAfter(":", animeId)
        val qualities = listOf("best", "1080p", "720p", "480p")
        val streams = mutableListOf<ProviderStream>()
        for (quality in qualities) {
            val root = request(
                "/episode_url?show_id=${encode(showId)}&ep_no=$episodeNumber&mode=sub&quality=$quality"
            ) ?: continue
    ...
    private fun array(root: JSONObject): List<JSONObject> {
        for (key in listOf("results", "data", "items", "episodes")) {
            root.optJSONArray(key)?.let { return it.objects() }
        }
        return if (root.length() > 0) listOf(root) else emptyList()
    }
        ...
        when (value) {
            is JSONObject -> {
                ...
                while (keys.hasNext()) {
                    ...
                    if (item is String && item.startsWith("http", true)) out += stream(item, quality)
                    else collectUrls(item, quality, out)
                }
            }
    ...
    private fun firstString(root: JSONObject, vararg keys: String): String? =
        keys.firstNotNullOfOrNull { root.optString(it).trim().ifBlank { null } }

    private fun stringArray(root: JSONObject, vararg keys: String): List<String> {
        for (key in keys) root.optJSONArray(key)?.let { return it.strings() }
        return emptyList()
    }
