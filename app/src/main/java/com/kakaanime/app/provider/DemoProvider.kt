...
class DemoProvider : AnimeProvider {
    ...
    override val name =
        "KakaAnime Demo Provider"

    override val priority =
        1

    private val demoAnime =
        ProviderAnime(
            id = "demo-one-piece",
            ...
            description =
                "Demo anime untuk menguji Provider Engine KakaAnime.",
    ...
    override suspend fun search(
        ...
        if (query.isBlank()) {
            return listOf(demoAnime)
        }

        return if (
            demoAnime.title.contains(
                query,
                ignoreCase = true
            )
        ) {
