...
internal class OtakudesuWebSource {
    ...
    suspend fun getAnime(slug: String): WebAnime? {
        for (source in sources) for (url in source.detailUrls(slug)) {
            ...
            if (!html.contains("One Piece", true) && !html.contains("anime", true)) continue
            val title = html.firstMatch("<h1[^>]*>(.*?)</h1>", "<title[^>]*>(.*?)</title>")?.stripHtml()?.removeSuffix(" - Otaku desu")?.trim()
    ...
    fun discoverPlaybackUrls(html: String, pageUrl: String): List<String> {
        ...
        listOf(
            ...
            "<source[^>]+src=[\\\"']([^\\\"']+)[\\\"']",
            "<video[^>]+src=[\\\"']([^\\\"']+)[\\\"']"
        ).forEach { pattern -> Regex(pattern, RegexOption.IGNORE_CASE).findAll(html).forEach { m -> resolve(pageUrl, decodeHtml(m.groupValues[1].trim()))?.let { urls += it } } }
        return urls.toList()
    }
    ...
    private fun parseEpisodes(pageUrl: String, html: String): List<WebEpisode> {
        ...
        for (match in pattern.findAll(html)) {
            ...
            val thumbnail = extractThumbnail(match.groupValues[0], pageUrl) ?: extractThumbnailAround(html, match.range.first, pageUrl)
            result += WebEpisode(url, number, text.ifBlank { "Episode $number" }, thumbnail)
        }
        return result.distinctBy { it.number }.sortedBy { it.number }
    }

    private fun extractThumbnail(anchorHtml: String, pageUrl: String): String? {
        val patterns = listOf(
            "(?:data-thumb|data-thumbnail|data-image|data-poster)=[\\\"']([^\\\"']+)[\\\"']",
            "<img[^>]+(?:src|data-src)=[\\\"']([^\\\"']+)[\\\"']"
        )
        for (pattern in patterns) Regex(pattern, RegexOption.IGNORE_CASE).find(anchorHtml)?.groupValues?.getOrNull(1)?.let { return resolve(pageUrl, decodeHtml(it)) }
        return null
    }

    private fun extractThumbnailAround(html: String, position: Int, pageUrl: String): String? {
        val start = (position - 900).coerceAtLeast(0)
        val end = (position + 1400).coerceAtMost(html.length)
        return extractThumbnail(html.substring(start, end), pageUrl)
    }
    ...
    private suspend fun get(url: String): String? = withContext(Dispatchers.IO) {
        runCatching {
            Request.Builder().url(url)
                ...
                .let { request -> client.newCall(request).execute().use { response -> if (response.isSuccessful) response.body?.string() else null } }
        }.getOrNull()
    }

    private fun WebSource.detailUrls(slug: String): List<String> {
        val clean = slug.removePrefix("otakudesu:").trim('/')
        val fitSlug = clean.replace(Regex("-(?:subtitle-)?indonesia$", true), "").replace(Regex("-sub-indo$", true), "").replace(Regex("^1piece$", true), "one-piece").replace(Regex("^onepiece$", true), "one-piece")
        return if (seriesStyle) listOf("$baseUrl/series/${fitSlug.trim('/')}/") else listOf("$baseUrl/anime/${clean.trim('/')}/", "$baseUrl/anime/${clean.trim('/').removeSuffix("-sub-indo")}/").distinct()
    }
    ...
    private fun extractEpisodeNumber(title: String, href: String): Int? { val value = "$title $href"; return Regex("(?:episode|eps|ep)[^0-9]*(\\d+)", true).find(value)?.groupValues?.getOrNull(1)?.toIntOrNull() ?: Regex("(?:-|/)(\\d+)(?:-|/|$)").find(href)?.groupValues?.getOrNull(1)?.toIntOrNull() }
