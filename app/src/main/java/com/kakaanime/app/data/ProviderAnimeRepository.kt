...
class ProviderAnimeRepository(
    ...
    override suspend fun getEpisodes(animeId: String): List<EpisodeData> {
    ...
    suspend fun getEpisodeStreams(
        animeId: String,
        episodeNumber: Int
    ): NormalizedEpisodeStream = engine.getEpisodeStreams(animeId, episodeNumber)
    ...
    ): NormalizedStream? = engine.getBestStream(
        animeId,
        episodeNumber,
        preferredQuality,
        isPremium
