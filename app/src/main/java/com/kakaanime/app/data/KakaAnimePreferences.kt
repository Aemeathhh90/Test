...
class KakaAnimePreferences(context: Context) {
    ...
    fun loadWatchedEpisodes(): Map<String, Int> {
        ...
        return runCatching {
            ...
            buildMap { json.keys().forEach { title -> put(title, json.optInt(title, 0)) } }
                .filterValues { it > 0 }
            ...
            buildList {
                for (index in 0 until array.length()) {
                    ...
                    add(WatchHistoryEntry(
                        ...
                        episodeThumbnailUrl = item.optString("episodeThumbnailUrl").takeIf { it.isNotBlank() },
    ...
    fun recordWatchedEpisode(
        ...
        val entry = WatchHistoryEntry(
            title = title,
            episode = episode,
            episodeTitle = episodeTitle ?: old?.episodeTitle,
            episodeThumbnailUrl = episodeThumbnailUrl ?: old?.episodeThumbnailUrl,
            watchedAt = now,
            durationMs = if (durationMs > 0L) durationMs else old?.durationMs ?: 0L
        )
    ...
    private fun saveWatchHistory(entries: List<WatchHistoryEntry>) {
        ...
        entries.forEach { entry ->
            array.put(JSONObject()
                ...
                .put("episodeThumbnailUrl", entry.episodeThumbnailUrl.orEmpty())
