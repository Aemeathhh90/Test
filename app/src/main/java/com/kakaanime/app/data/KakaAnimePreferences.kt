...
class KakaAnimePreferences(context: Context) {
    ...
    fun setFavoriteGroup(animeGroupId: String, favorite: Boolean) { val key = animeGroupId.trim(); if (key.isBlank()) return; val current = loadFavoriteGroupIds().toMutableSet(); if (favorite) current.add(key) else current.remove(key); saveFavoriteGroupIds(current) }
    fun loadWatchedEpisodes(): Map<String, Int> { val raw = prefs.getString(KEY_WATCHED_EPISODES, null) ?: return emptyMap(); return runCatching { val json = JSONObject(raw); buildMap { json.keys().forEach { title -> put(title, json.optInt(title, 0)) } }.filterValues { it > 0 } }.getOrDefault(emptyMap()) }
    ...
    fun recordWatchedEpisode(identity: AnimeStateIdentity, episode: Int) { if (episode <= 0 || identity.animeGroupId.isBlank()) return; val key = watchedSeasonKey(identity); val current = loadWatchedEpisodesSeasonAware().toMutableMap(); current[key] = episode; saveWatchedEpisodesSeasonAware(current) }
    ...
    fun markEpisodeUnlocked(identity: AnimeStateIdentity, episode: Int) { if (episode <= 0 || identity.animeGroupId.isBlank()) return; saveUnlockedEpisodesSeasonAware(loadUnlockedEpisodesSeasonAware() + identity.episodeKey(episode)) }
    ...
    private fun watchedSeasonKey(identity: AnimeStateIdentity): String = buildString { append(identity.animeGroupId.trim()); append("::season:"); append(identity.seasonNumber ?: "na"); append("::title:"); append(identity.seasonTitle?.trim()?.lowercase().orEmpty()) }
    fun loadWatchHistory(): List<WatchHistoryEntry> { val raw = prefs.getString(KEY_WATCH_HISTORY, null) ?: return emptyList(); return runCatching { val array = JSONArray(raw); buildList { for (i in 0 until array.length()) { val item = array.optJSONObject(i) ?: continue; val title = item.optString("title").trim(); val episode = item.optInt("episode", 0); if (title.isBlank() || episode <= 0) continue; add(WatchHistoryEntry(title, episode, item.optString("episodeTitle").takeIf { it.isNotBlank() }, item.optString("episodeThumbnailUrl").takeIf { it.isNotBlank() }, item.optLong("watchedAt", 0L), item.optLong("durationMs", 0L))) } }.sortedByDescending { it.watchedAt } }.getOrDefault(emptyList()) }
    fun loadWatchHistorySeasonAware(): List<SeasonAwareWatchHistoryEntry> { val raw = prefs.getString(KEY_WATCH_HISTORY_SEASON_AWARE, null) ?: return emptyList(); return runCatching { val array = JSONArray(raw); buildList { for (i in 0 until array.length()) { val item = array.optJSONObject(i) ?: continue; val groupId = item.optString("animeGroupId").trim(); val episode = item.optInt("episode", 0); if (groupId.isBlank() || episode <= 0) continue; add(SeasonAwareWatchHistoryEntry(groupId, item.optInt("seasonNumber").takeIf { item.has("seasonNumber") && !item.isNull("seasonNumber") }, item.optString("seasonTitle").takeIf { it.isNotBlank() }, episode, item.optString("title").takeIf { it.isNotBlank() }, item.optString("episodeTitle").takeIf { it.isNotBlank() }, item.optString("episodeThumbnailUrl").takeIf { it.isNotBlank() }, item.optLong("watchedAt", 0L), item.optLong("durationMs", 0L))) } }.sortedByDescending { it.watchedAt } }.getOrDefault(emptyList()) }
    fun recordWatchHistory(identity: AnimeStateIdentity, title: String, episode: Int, episodeTitle: String? = null, episodeThumbnailUrl: String? = null, durationMs: Long = 0L) { if (episode <= 0 || identity.animeGroupId.isBlank()) return; val current = loadWatchHistorySeasonAware().toMutableList(); val normalizedSeasonTitle = normalizeSeasonTitle(identity.seasonTitle); val index = current.indexOfFirst { it.animeGroupId == identity.animeGroupId && it.seasonNumber == identity.seasonNumber && normalizeSeasonTitle(it.seasonTitle) == normalizedSeasonTitle && it.episode == episode }; val old = current.getOrNull(index); val entry = SeasonAwareWatchHistoryEntry(identity.animeGroupId.trim(), identity.seasonNumber, identity.seasonTitle?.trim()?.takeIf { it.isNotBlank() }, episode, title, episodeTitle ?: old?.episodeTitle, episodeThumbnailUrl ?: old?.episodeThumbnailUrl, System.currentTimeMillis(), if (durationMs > 0L) durationMs else old?.durationMs ?: 0L); if (index >= 0) current[index] = entry else current.add(entry); saveWatchHistorySeasonAware(current.take(MAX_WATCH_HISTORY_ENTRIES)) }
    fun deleteWatchHistoryEntry(identity: AnimeStateIdentity, episode: Int) { val normalizedSeasonTitle = normalizeSeasonTitle(identity.seasonTitle); saveWatchHistorySeasonAware(loadWatchHistorySeasonAware().filterNot { it.animeGroupId == identity.animeGroupId && it.seasonNumber == identity.seasonNumber && normalizeSeasonTitle(it.seasonTitle) == normalizedSeasonTitle && it.episode == episode }) }
    fun clearWatchHistorySeasonAware() { saveWatchHistorySeasonAware(emptyList()) }
    fun recordWatchedEpisode(title: String, episode: Int, episodeTitle: String? = null, episodeThumbnailUrl: String? = null, durationMs: Long = 0L) { val current = loadWatchHistory().toMutableList(); val now = System.currentTimeMillis(); val index = current.indexOfFirst { it.title == title && it.episode == episode }; val old = current.getOrNull(index); val entry = WatchHistoryEntry(title, episode, episodeTitle ?: old?.episodeTitle, episodeThumbnailUrl ?: old?.episodeThumbnailUrl, now, if (durationMs > 0L) durationMs else old?.durationMs ?: 0L); if (index >= 0) current[index] = entry else current.add(entry); saveWatchHistory(current.take(MAX_WATCH_HISTORY_ENTRIES)) }
    ...
    fun loadThemeMode(): String = prefs.getString(KEY_THEME_MODE, "DARK") ?: "DARK"
    fun saveThemeMode(value: String) { prefs.edit().putString(KEY_THEME_MODE, value).apply() }
    fun loadDarkMode(): Boolean = loadThemeMode() == "DARK"
    fun saveDarkMode(value: Boolean) { saveThemeMode(if (value) "DARK" else "LIGHT") }
...
data class SeasonAwareWatchHistoryEntry(val animeGroupId: String, val seasonNumber: Int?, val seasonTitle: String?, val episode: Int, val title: String?, val episodeTitle: String?, val episodeThumbnailUrl: String?, val watchedAt: Long, val durationMs: Long) { val identity: AnimeStateIdentity get() = AnimeStateIdentity(animeGroupId, seasonNumber, seasonTitle) }
