...
class AniListCalendarService {
    ...
    suspend fun getSchedule(
            ...
            while (hasNextPage && page <= 5) {
                val pageEntries = runCatching {
                    val payload = JSONObject()
                        .put("query", query)
                        .put("variables", JSONObject().put("page", page).put("from", from.toInt()).put("to", to.toInt()))
        ...
        }.distinctBy { "${it.id}-${it.episode}-${it.airingAt}" }.sortedBy { it.airingAt }
    }
    ...
    private fun parse(body: String): Pair<List<AniListScheduleEntry>, Boolean> {
        ...
        val entries = buildList {
            for (i in 0 until schedules.length()) {
                ...
                val rawTitle = sequenceOf("english", "romaji", "native").mapNotNull { title.optString(it).trim().takeIf(String::isNotBlank) }.firstOrNull() ?: continue
                ...
                val genres = buildList {
                    ...
                    for (index in 0 until values.length()) values.optString(index).trim().takeIf { it.isNotBlank() }?.let(::add)
                }
                add(AniListScheduleEntry(
                    id = mediaId,
                    title = displayTitle,
                    episode = episode,
                    airingAt = airingAt,
                    imageUrl = cover?.optString("large")?.takeIf { it.isNotBlank() } ?: cover?.optString("medium")?.takeIf { it.isNotBlank() },
    ...
    private fun startOfTodayEpochSecond(): Long = LocalDate.now().atStartOfDay(ZoneId.systemDefault()).toEpochSecond()
}
